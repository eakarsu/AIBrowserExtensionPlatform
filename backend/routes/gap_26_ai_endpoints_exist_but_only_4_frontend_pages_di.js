...
// Feature: 26 AI endpoints exist but only 4 frontend pages — discoverability gap
  ...
  try {
    ...
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({
        ...
        feature: 'gap_26_ai_endpoints_exist_but_only_4_frontend_pages_di',
      });
    }
    const payload = req.body || {};
    const userPrompt = `Context (JSON):\n${JSON.stringify(payload).slice(0, 4000)}\n\nTask: ${ '26 AI endpoints exist but only 4 frontend pages — discoverability gap' }\n\nReturn JSON: {"summary": string, "actions": [string], "risks": [string], "confidence": number_0_to_1}.`;
    const system = `You are an expert assistant for the feature: '26 AI endpoints exist but only 4 frontend pages — discoverability gap'. Respond with strict JSON.`;
    ...
    const out = { feature: 'gap_26_ai_endpoints_exist_but_only_4_frontend_pages_di', model: aiResp?.model, parsed, raw: content };
    if (dbPool) {
      try {
        await dbPool.query(
          'INSERT INTO gap_features (feature_key, user_id, input_payload, output_payload) VALUES ($1, $2, $3, $4)',
          ['gap_26_ai_endpoints_exist_but_only_4_frontend_pages_di', req.user?.id || null, JSON.stringify(payload), JSON.stringify(out)]
        );
...
router.get('/health', (req, res) => res.json({ feature: 'gap_26_ai_endpoints_exist_but_only_4_frontend_pages_di', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
...
router.get('/history', async (req, res) => {
  ...
  try {
    ...
    const r = await dbPool.query(
      ...
      ['gap_26_ai_endpoints_exist_but_only_4_frontend_pages_di']
    );
