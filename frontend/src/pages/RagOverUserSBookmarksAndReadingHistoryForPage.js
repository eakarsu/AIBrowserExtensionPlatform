...
// Feature: RAG over user's bookmarks and reading history for personalized recall
import React, { useState } from 'react';
import api from '../services/api';

export default function RagOverUserSBookmarksAndReadingHistoryForPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-rag-over-user-s-bookmarks-and-reading-history-for-/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>RAG over user's bookmarks and reading history for personalized recall</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-rag-over-user-s-bookmarks-and-reading-history-for-/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
