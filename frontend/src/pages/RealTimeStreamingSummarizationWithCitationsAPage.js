...
// Feature: Real-time streaming summarization with citations as user scrolls
import React, { useState } from 'react';
import api from '../services/api';

export default function RealTimeStreamingSummarizationWithCitationsAPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-real-time-streaming-summarization-with-citations-a/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Real-time streaming summarization with citations as user scrolls</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-real-time-streaming-summarization-with-citations-a/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
