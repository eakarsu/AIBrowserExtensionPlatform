...
// Feature: White-label SDK for SaaS vendors to embed AI sidekick in their web apps
import React, { useState } from 'react';
import api from '../services/api';

export default function WhiteLabelSdkForSaasVendorsToEmbedAiSidekPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-white-label-sdk-for-saas-vendors-to-embed-ai-sidek/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>White-label SDK for SaaS vendors to embed AI sidekick in their web apps</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-white-label-sdk-for-saas-vendors-to-embed-ai-sidek/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
