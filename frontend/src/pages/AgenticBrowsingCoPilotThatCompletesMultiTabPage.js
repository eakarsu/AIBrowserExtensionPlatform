...
// Feature: Agentic browsing co-pilot that completes multi-tab tasks (research, comparison, form-filling)
import React, { useState } from 'react';
import api from '../services/api';

export default function AgenticBrowsingCoPilotThatCompletesMultiTabPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-agentic-browsing-co-pilot-that-completes-multi-tab/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic browsing co-pilot that completes multi-tab tasks (research, comparison, form-filling)</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-browsing-co-pilot-that-completes-multi-tab/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
