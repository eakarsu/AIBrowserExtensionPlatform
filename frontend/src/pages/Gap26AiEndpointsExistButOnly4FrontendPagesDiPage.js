...
export default function Gap26AiEndpointsExistButOnly4FrontendPagesDiPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-26-ai-endpoints-exist-but-only-4-frontend-pages-di/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>26 AI endpoints exist but only 4 frontend pages — discoverability gap</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-26-ai-endpoints-exist-but-only-4-frontend-pages-di/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
